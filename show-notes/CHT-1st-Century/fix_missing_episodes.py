...
def extract_clean_title(raw_title):
    """Extract clean episode title from raw YouTube title."""
    # Remove CHT prefix variations
    title = re.sub(r'^CHT\s+\|\s+', '', raw_title)
    title = re.sub(r'^CHT\s+', '', raw_title)
    
    # Remove episode code from title
    title = re.sub(r'^S\d+E\d+:\s*', '', title)
    title = re.sub(r'^S\d+E\d+\s+', '', title)
    
    # Clean up Resource prefix
    title = re.sub(r'^Resource:\s*', 'Resource: ', title)
    
    return title.strip()

def extract_episode_code(raw_title):
    """Extract S2EXX code from title."""
    match = re.search(r'(S\d+E\d+)', raw_title)
...
def main():
    ...
    for txt_path in txt_files:
        ...
        # Check if markdown already exists for this episode
        expected_filename = f"{episode_code.lower()}-*.md"
        existing = glob.glob(os.path.join(BASE_DIR, expected_filename))
        if existing:
            print(f"Skipping {episode_code} - already exists")
            continue
        
        # Get metadata
        print(f"Processing {episode_code} ({video_id})...")
