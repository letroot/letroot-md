...
def split_sentences(text):
    ...
    # If that didn't split much, try split on any period followed by space
    if len(sents) < 5:
        sents = re.split(r'(?<=[.!?])\s+', text)
    
    # If still not split enough, split on commas for ASR text
    if len(sents) < 5:
        # Split on sentence-like patterns: period, question, exclamation followed by any word
        sents = re.split(r'(?<=[.!?])\s+(?=[a-zA-Z])', text)
    
    # Last resort: split on long pauses indicated by "and" or "so" at sentence start
    if len(sents) < 10:
        sents = re.split(r'\.\s+(?=[A-Z])', text)
    
    return [s.strip() for s in sents if s.strip()]

def insert_periods_for_asr(text):
    """Add periods where there are clear sentence boundaries in ASR text."""
    # Add period before Bible references like "acts chapter"
    text = re.sub(r'(\w)\s+(acts|genesis|exodus|leviticus|numbers|deuteronomy|joshua|judges|ruth|1 samuel|2 samuel|1 kings|2 kings|1 chronicles|2 chronicles|ezra|nehemiah|esther|job|psalms|psalm|proverbs|ecclesiastes|isaiah|jeremiah|ezekiel|daniel|hosea|joel|amos|obadiah|jonah|micah|nahum|habakkuk|zephaniah|haggai|zechariah|malachi|matthew|mark|luke|john|romans|1 corinthians|2 corinthians|galatians|ephesians|philippians|colossians|1 thessalonians|2 thessalonians|1 timothy|2 timothy|titus|philemon|hebrews|james|1 peter|2 peter|1 john|2 john|3 john|jude|revelation)\s+chapter', r'\1. \2 chapter', text, flags=re.IGNORECASE)
    
    # Add period before common transition phrases
    text = re.sub(r'(\w)\s+(now|so|well|okay|now then|alright|first|second|third|next|finally|also|furthermore|moreover|however|therefore|thus|hence|because|since|but|and|or|if|when|while)', r'\1. \2', text, flags=re.IGNORECASE)
    
    return text

def detect_headings(sentences):
    """Detect content-based section headings from sentence transitions."""
    breaks = []
    ...
    for i, sent in enumerate(sentences):
        ...
        # Look for Bible references at start of sentence
        for book in bible_books:
            pat = rf'^(?:the\s+)?{re.escape(book)}\s+(?:chapter\s+)?\d+'
            if re.match(pat, low):
                ref = re.match(pat, low).group(0)
                ref = ref.replace('the ', '').title()
                breaks.append((i, f"## {ref}"))
                break
        else:
            # Look for explicit topic transition phrases
            transitions = [
                (r"^(?:now|so|well|okay)\s+let['']*(?:us|'?s)?\s+(?:look\s+at|turn\s+to|go\s+to|consider|delve\s+into|examine|read|jump\s+to|open)\s+(?:the\s+)?([^.]{3,35})", "topic"),
                (r"^(?:now|so|well|okay)\s+this\s+brings\s+us\s+to\s+([^.]{3,35})", "topic"),
                (r"^(?:now|so|well|okay)\s+we\s+(?:come\s+to|arrive\s+at|turn\s+to)\s+(?:the\s+)?([^.]{3,35})", "topic"),
                (r"^(?:now|so|well|okay)\s+i\s+want\s+to\s+(?:talk\s+about|look\s+at|share|show\s+you|read)\s+(?:the\s+)?([^.]{3,35})", "topic"),
                (r"^(?:now|so|well|okay)\s+as\s+we\s+(?:look\s+at|consider|examine|delve\s+into)\s+(?:the\s+)?([^.]{3,35})", "topic"),
                (r"^(?:the\s+)?first\s+thing\s+([^.]{3,35})", "topic"),
                (r"^(?:the\s+)?next\s+thing\s+([^.]{3,35})", "topic"),
                (r"^moving\s+on\s+to\s+([^.]{3,35})", "topic"),
                (r"^let['']*(?:us|'?s)?\s+look\s+at\s+(?:the\s+)?([^.]{3,35})", "topic"),
                (r"^let['']*(?:us|'?s)?\s+consider\s+([^.]{3,35})", "topic"),
            ]
...
def format_paragraphs(sents, max_s=3):
...
def process(filepath):
    ...
    text = insert_periods_for_asr(text)  # Add periods before processing sentences
