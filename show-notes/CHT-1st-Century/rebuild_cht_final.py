...
def clean_text(text):
    """Clean only obvious ASR artifacts as specified in requirements."""
    # Remove [Music] tags
    text = re.sub(r'\[Music\]', '', text, flags=re.IGNORECASE)
    
    # Remove filler words (single sounds)
    ...
    # Fix broken words (e.g., "e- xperience" -> "experience")
    text = re.sub(r'(\w)-\s+(\w)', r'\1\2', text)
    
    # Fix bad contractions (e.g., "don ' t" -> "don't")
...
def detect_headings(sentences):
    ...
    for i, sent in enumerate(sentences):
        ...
        for book in bible_books:
            ...
            # Look for topic transition phrases
            transitions = [
                (r"^(?:now\s+)?let['']*(?:us|'?s)?\s+(?:look\s+at|turn\s+to|go\s+to|consider|delve\s+into|examine|read|jump\s+to|open)\s+(?:the\s+)?([^.]{5,40})", "topic"),
                (r"^(?:now\s+)?this\s+brings\s+us\s+to\s+([^.]{5,40})", "topic"),
                (r"^(?:now\s+)?we\s+(?:come\s+to|arrive\s+at|turn\s+to)\s+(?:the\s+)?([^.]{5,40})", "topic"),
                (r"^(?:now\s+)?i\s+want\s+to\s+(?:talk\s+about|look\s+at|share|show\s+you|read)\s+(?:the\s+)?([^.]{5,40})", "topic"),
                (r"^(?:now\s+)?as\s+we\s+(?:look\s+at|consider|examine|delve\s+into)\s+(?:the\s+)?([^.]{5,40})", "topic"),
            ]
